...
if controlTime_des==100 %300 
    ...
    for i=1:NS
        min=( ( apos(1,1)-sinfo(i,1)+apos_shift(1) )^2+( apos(1,2)-sinfo(i,2)+apos_shift(2))^2 );
        mini=1;
        for j=2:NA
            temp=( ( apos(j,1)-sinfo(i,1)+apos_shift(1) )^2+( apos(j,2)-sinfo(i,2)+apos_shift(2) )^2 );
            if temp < min
                min=temp;
                mini=j;
            end
...
%             if((sinfo(p(i,j),5) > 0.005)|(sinfo(p(i,j),5) < -0.005))
        ...
        if(sumd == 0)
            des(i,1)=apos(i,1)-apos_shift(1);
            des(i,2)=apos(i,2)-apos_shift(2);
        else
            des(i,1)=sumx/sumd-apos_shift(1);
            des(i,2)=sumy/sumd-apos_shift(2);
        end
